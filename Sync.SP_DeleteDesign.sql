create or alter proc Sync.SP_DeleteDesign(@FormID int)
as
begin
Declare	    @Count					Int =	0
...
BEGIN TRY
	...
	BEGIN TRANSACTION
			...
			Begin
				...
				Select @FormVersionIDCount	=	Count(distinct FormDesignVersionID)
				From   Temp.FormVersionList
				
			--select * from @FormDesignVersionIDList
			--select @FormVersionIDCount as fvidcount
			--select  * from Temp.FormVersionList
				...
				---- once we get data for provided form design version then started deleting from below table for that version
						...
						while(@FormVersionIDCount>0)
						begin
							Delete
							From		UI.FormDesignVersion
							where FormDesignVersionID	= (select min(FormDesignVersionID) from Temp.FormVersionList)

							update Temp.FormVersionList
							set FormDesignVersionID = NULL
							where FormDesignVersionID	= (select min(FormDesignVersionID) from Temp.FormVersionList)

							Set	@FormVersionIDCount	=	@FormVersionIDCount - 1
						end
			...
			If Exists(select 1 from @FormDesignVersionIDList)
			Begin
